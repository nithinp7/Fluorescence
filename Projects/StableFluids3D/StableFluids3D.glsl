...
vec3 raymarch(vec3 pos, vec3 dir, inout uvec2 seed) {
  ...
  for (uint iter = 0; iter < RAYMARCH_ITERS; iter++) {
    ...
    if (length(density) > 0.0)
      outLight += 
          phaseFunction(dot(lightDir, dir), 0.0) *
          vec3(LIGHT_STRENGTH) * lightThroughput * throughput;
...
void PS_Display() {
  if (RENDER_MODE == 0) {
    ...
    outColor = vec4(raymarch(pos, dir, jitterSeed), 1.0);
  } else {
